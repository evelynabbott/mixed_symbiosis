...
lnames = load(file = "resample25k_C_v3b.RData")
...
lnames = load(file = "WGCNA/C_resampled_wgcna.RData")
...
table(moduleColors)
coul <- coul <- rev(colorRampPalette(brewer.pal(8, "RdYlBu"))(25))
pdf("evelyn_plot_C_resampled_v3b.pdf",height=3,width=5)
pheatmap(t(MEsm),scale="row",cluster_cols=FALSE, labels_col = "",color = blueWhiteRed(50),border_color=NA)
dev.off()
pdf("evelyn_plot_C_resampled_v3b_Dprop.pdf",height=3,width=5)
plot(datTraits$Dprop[order(datTraits$Dprop)],xaxt="na",bty="n",xlab="",ylab="proportion of\nDurusdinium",type="l")
dev.off()
